...
async def upload_ncp_pdf(file: UploadFile = File(...)):
    ...
    # Flexible Regex for Victim / Complainant / Name
    victim_name = get_val(r"(?:Victim|Complainant|Name)[\s:]+([A-Za-z\s]{3,30})", extracted_text)
    
    # If regex missed, grab the first clean non-empty line instead of a hardcoded name
    if not victim_name:
        lines = [line.strip() for line in extracted_text.splitlines() if line.strip()]
        victim_name = lines[0] if lines else "Unknown Victim"
    ...
    victim_phone = phones[0] if len(phones) > 0 else ""
    suspect_phone = phones[1] if len(phones) > 1 else ""
